...
const DB_PATH = path.join(DATA_DIR, "vidpod.db");
...
export function listAdsFromDb(): AdRow[] {
  return getDb().prepare("SELECT id, name, filename, duration FROM ads ORDER BY name").all() as AdRow[];
}

export function upsertAd(id: string, name: string, filename: string, duration = 15): void {
  getDb()
    .prepare(
